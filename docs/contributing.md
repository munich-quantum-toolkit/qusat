...
Thank you for your interest in contributing to MQT QuSAT! This document outlines
the development guidelines and how to contribute.

We use GitHub to [host code](https://github.com/munich-quantum-toolkit/qusat),
to [track issues and feature requests][issues], as well as accept
[pull requests](https://github.com/munich-quantum-toolkit/qusat/pulls). See
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/qusat/issues>. Include steps to
...
- 💡 Propose features:
  ...
  <https://github.com/munich-quantum-toolkit/qusat/issues>. Describe the
...
which contains context and instructions to help the agent work on MQT QuSAT. For
...
[GitHub Actions page](https://github.com/munich-quantum-toolkit/qusat/actions/workflows/ci.yml).
...
{code}`-DBUILD_MQT_QUSAT_TESTS=OFF` to the CMake configure step.
:::

Our CI pipeline on GitHub also collects code coverage information and uploads it
to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/qusat). Our goal is to
...
To build only the Python bindings, pass {code}`-DBUILD_MQT_QUSAT_BINDINGS=ON` to
...
in the {code}`python/mqt/qusat` directory. These stub files are auto-generated.
...
a breaking change for MQT QuSAT. If you are sure that the dependency update does
not introduce any breaking changes for MQT QuSAT, you can remove the
...
[Releases page](https://github.com/munich-quantum-toolkit/qusat/releases) on
